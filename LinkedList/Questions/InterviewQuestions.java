...
public class InterviewQuestions {
    ...
    public ListNode detectCycle(ListNode head) {
        ...
        while (fast != null && fast.next != null) {
            fast = fast.next.next; //2 step motion
            slow = slow.next;  // 1 step motion
        ...
        ListNode f = head;
        ListNode s = head;
        //  Moving s pointer to the length of the cycle
        // so that they are at the same distance from the start of the cycle
        while (length > 0) {
            s = s.next;
            length--;
        }
